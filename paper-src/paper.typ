...
The problem of string comparison and pattern matching has been well-researched with various statements, requirements and backgrounds. One of the first steps in this area has been done by Knuth, Morris and Pratt @kmp, with advanced suffix structures and preprocessing algorithms to come in the next 40 years. This paper is aimed specifically to reinforce our knowledge about some simpler methods which can solve the variations of the problem efficiently, rather than to propose something specific and new.
...
The "check for equality" queries have the same format as above; the new query is denoted by $(p, x)$ ($1 <= p <= n$), meaning that $S_p$ has to be set to $x$, where $x$ is an arbitrary latin alphabet lowercase character.

== Dynamic range update substring comparison problem
The "update" query has format $(l, r, x)$, meaning that all elements $S_i$ where $l <= i <= r$ have to be set to $x$.
...
$"hash"(S, p)$ computes the polynomial hash of the first $p$ characters of $S$ (prefix of $S$). It is proven that no same hash values can be obtained from another values of $S$ and $p$. The reason why we define the prefix hash function here will be clear later as we generalise prefixes to substrings.

== Claim: the strings are equal if and only if their hash values are equal

\

The practical problem is that for long prefixes the hash values become very large and are hard to compare, and at a certain point the numbers become longer than the substrings being analysed. This affects time complexity. Luckily, there has been found a solution to this - using the $"hash"(S, p) "mod" q$, where $"mod"$ denotes the remainder operation. This creates a risk of a hash-collision (two different substrings have the same hash because of the birthday paradox) but makes hashes practical. From this point, the $"hash"(S, p)$ notation includes the remainder operation by a certain big prime number $q$.

\

In order to find the hash of a certain substring $[l, r)$, we need two values: $h_l = "hash"(S, l)$ and $h_r = "hash"(S, r)$. Then all is needed is multiplying $h_l$ by $b^(r - l)$ and subtracting that from $h_r$ (similarly to prefix sums). All the operations should be done $"mod" q$. The proof why this is true is left as an exercise for a reader.
